...
export class OrderResolver {
  ...
  async getOrder(@Ctx() ctx: Context, @Arg("id") id: number) {
    return await OrderService.find(id, ctx.user!.id);
  }

  @Authorized()
  @Mutation((returns) => Order)
  async updateOrderStatus(@Arg("id") id: number, @Arg("statusId") statusId: number, @Ctx() ctx: Context) {
    return await OrderService.updateStatus(id, statusId, ctx.user!.id);
  }

  @Authorized()
  @Mutation((returns) => Boolean)
  async cancelOrder(@Arg("id") id: number, @Ctx() ctx: Context) {
