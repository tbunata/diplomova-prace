...
import { UpdateOrderStatusInput } from "./interface";
...
ordersRouter.put("/:id/updateStatus", async (req: Request, res: Response) => {
  try {
    const id: number = parseInt(req.params.id, 10);
    const statusUpdate = new UpdateOrderStatusInput(req.body);

    const updatedOrder = await OrderService.updateStatus(id, statusUpdate.statusId, req.user.user_id);
...
ordersRouter.delete("/:id", async (req: Request, res: Response) => {
  try {
    ...
    await OrderService.cancelOrder(id, req.user.user_id);
